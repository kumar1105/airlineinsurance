package com.server.control;
	...
	public class healthclaim extends HttpServlet {
		Connection connect = com.servlet.jdbc.JDBC.getInstance();
		...
		protected void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException {
			...
			try {
				PreparedStatement prep= connect.prepareStatement("select * from insurance where policyid=?");
				prep.setString(1, policyid);
	            ...
	            for (int i = 1; i <= total; i++)
	         {
	            ...
	            while (rs.next ())
	         {
	             out.print ("<tr><td>" + rs.getInt (1) + "</td><td>" +  rs.getString (2) + " </td><td>" + rs.getInt (3) + "</td><td>"+rs.getInt(4)+"</td></tr>");
	              number= rs.getInt(3);
	         }
	            out.print ("</table>");
